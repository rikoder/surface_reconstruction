...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, depth_GT, GT_iters):
    ...
    if depth_GT:
        for idx, view in enumerate(pbar):
            ...
            np.save( os.path.join(depth_path,'depth_map_{0:05d}.npy'.format(idx)), depth_np)
            ...
            #
            # depth_diff1 = torch.maximum(normalized_gt_depth - normalized__predict_depth, torch.tensor(0.0))
            ...
            # depth_diff_normalized = depth_diff
            #
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, depth_GT, GT_iters, args):
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument('--depth_GT', required=False, nargs="+", type=str, default=['output_benchmarks/down4_200views_30000_sparsept_llff'])
    ...
    #args.depth_GT, args.GT_iters = None , None
    render_sets(model.extract(args), args.iteration[0], pipeline.extract(args), args.skip_train, args.skip_test,args.depth_GT, args.GT_iters, args)