...
def readColmapCameras_old(cam_extrinsics, cam_intrinsics, images_folder):
    cam_infos = []
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    # bin_path = os.path.join(path, str(n_views) + "_views/dense/sparse/points3D.bin")
    ply_path = os.path.join(path, "dense/fused.ply")
    # ply_path = "/home/rikhilgupta/Desktop/Compact_Gaussian_study/gaussian-splatting/merged_point_cloud.ply"
    try:
    ...
    if n_views > 0:
        ...
        train_cam_infos = []
        curr_img_dir = os.path.join(path, str(n_views) + "_views/dense/images")
        print("curr_img_dir==>",curr_img_dir)
        img_list = os.listdir(curr_img_dir)
        print(img_list)
        for x in img_list:
            for y in cam_infos:
                if x == y.image_name + '.JPG':
                    train_cam_infos.append(y)
 
    print("len of train_cam_infos==>",len(train_cam_infos))  # Total number of images
...
def readColmapSceneInfo_old(path, images, eval, llffhold=8):
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse/0", "images.bin")
        cameras_intrinsic_file = os.path.join(path, "sparse/0", "cameras.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
