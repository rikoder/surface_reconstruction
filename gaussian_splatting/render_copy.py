...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, depth_GT, GT_iters):
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument('--depth_GT', required=True, nargs="+", type=str, default=[])
    parser.add_argument('--GT_iters', required=True, nargs="+", type=str, default=[])
    ...
    render_sets(model.extract(args), args.iteration[0], pipeline.extract(args), args.skip_train, args.skip_test, args.depth_GT, args.GT_iters)